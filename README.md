...
Build and run the production container with a database reachable from Docker:

```bash
$ docker build -t restaurant-backend .
$ docker run --env-file .env -p 3000:3000 restaurant-backend
```

Pull requests and changes to `main` run lint for tests, build, unit tests, E2E
tests and a container build through GitHub Actions.
